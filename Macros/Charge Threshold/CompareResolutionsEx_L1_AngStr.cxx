...
void CompareResolutionsEx_L1_AngStr()
{
    ...
    int wantedIdx = FindSimuFileIndex(files, "3-5");
    ...
    // ========================================================================
    ROOT::RDataFrame df {
        "Final_Tree", TString::Format("../../PostAnalysis/Outputs/tree_ex_F_7Li_d_%s_filtered.root", particle.c_str())};
    auto def {df.Filter(
    ...
    auto hEx = def.Histo1D({nameEx.c_str(), titleEx.c_str(), 100, -2, 2}, "Ex");

    // ========================================================================
    // EX 1D: SIMULACION + EXPERIMENTO
    // ========================================================================
    double exGausFitMin {-2.};
    double exGausFitMax {1.};
    ...
    for(int i = 0; i < nTLbands_Simu; i++)
    {
        ...
        histsExByTL_Simu.push_back(
            defBand_Simu.Histo1D({nameExSimuBand.c_str(), titleExSimuBand.Data(), 100, -2, 2}, "Eex"));
    }

    gROOT->cd(); // FIX
    auto c_Ex_TL_Simu =
        new TCanvas("c_Ex_TL_Simu", "Ex simulation (3-5 AngStr) por franjas de TL - Ajustes gaussianos", 1600, 1000);
