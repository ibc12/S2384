...
void GetWorkingTimeForEachSilicon(std::string beam)
{
    // Get Sil data
    std::string dataconf {};
    if(beam == "11Li")
    ...
    // ---------------------------------------------------
    auto hRuns = gated.Histo1D({"hRuns", "Beam counts per run", 122 - 19 + 1, 18.5, 122.5}, "fRun");
    ...
    // ---------------------------------------------------
    std::string outfileName {"./Outputs/detector_counts_" + beam + ".txt"};
    ...
    for(auto det : {"f0", "r0", "l0"})
    {
        for(int i = 0; i <= 11; i++)
        {
            ...
            for(auto& [run, c] : beamCounts)
            {
                ...
                if((det == std::string("l0") && i == 9) || (det == std::string("f0") && i == 2) ||
