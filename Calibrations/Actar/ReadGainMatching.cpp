            ...
            for(int hit = 0, otherSize = evt->CoboAsad[it].peakheight.size(); hit < otherSize; hit++)
            {
                if((yval != -1) && (xval != -1))
                {
                    ...
                    if(z_position > 0.)
                    {
                        ...
                        // Fill histogram
                        if(Qiaux > 500) // in this experiment all runs have a baseline that we eliminate
                        {
                            h->Fill(where, Qiaux);
                        }
                    }
...
void ReadGainMatching(bool isMatched = true){
    ...
    std::vector<int> runs {113, 114, 115, 116, 117, 118, 119}; // Put whichever runs needed (this was for e796)
    ...
    calman.ReadLookUpTable("../Actar/LT.dat"); // convert LT file in a string line
    if(isMatched)
        calman.ReadPadAlign("./Outputs/pad_align_coef_v2022_V5.cal");
